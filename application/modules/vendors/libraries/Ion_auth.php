...
class Ion_auth
{
	...
	public function is_customer($id = FALSE)
	{
		...
		$customer_group = $this->config->item('default_group', 'ion_auth');

		return $this->ion_auth_model->in_group($customer_group, $id);
	}
	 ...
	 * @author Desmond Njuguna
	 */
	public function is_wholesaler($id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_wholesaler');

		$wholesaler_group = $this->config->item('wholesaler_group', 'ion_auth');

		return $this->ion_auth_model->in_group($wholesaler_group, $id);
	}
	...
	public function is_vendor($id = FALSE)
	{
		...
		$vendor_group = $this->config->item('vendor_group', 'ion_auth');

		return $this->ion_auth_model->in_group($vendor_group, $id);
	}
